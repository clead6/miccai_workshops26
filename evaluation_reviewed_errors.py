...
def testing_reviewed(testing_dataset_original_loader, testing_dataset_corrected_loader,
                     args, diffusion, ema, vae, test_t, device):
    ...
    for batch_idx, (data_original, data_corrected) in enumerate(zip(testing_dataset_original_loader, testing_dataset_corrected_loader)):
        ...
        for im in range(len(keys)):
            ...
            if batch_idx < num_saved_batches:
                if diffusion is not None:
                    plot_name = f"test_{keys[im]}_t{test_t}.png"
                    plot_title = f"TEST t={test_t}"
                else:
    ...
    if has_image:
        ...
        if diffusion is not None:
            print(f"[IMAGE] MSE t={test_t}:")    
        else:
            print(f"[IMAGE]:")
        print(f"   MSE: Original: {m_ori:.4f} +- {s_ori:.4f}, Corrected: {m_cor:.4f} +- {s_cor:.4f}")

        img_metrics_save = {
            "mses": mses
        }
        if diffusion is not None:
            img_metrics_save['test_t'] = test_t
            filename = f"img_metrics_reviewed_{args['dataset']}_{args['test_sampling']}.json"
        else:
    ...
    if has_seg:     
        ...
        if diffusion is not None:
            seg_metrics_save['test_t'] = test_t
            filename = f"seg_metrics_reviewed_{args['dataset']}_{args['test_sampling']}.json"
        else:
...
def main():
    ...
    print(f"Running evaluation with test_t={test_t}")
    ...
    if args['model'] == 'DDPM' or args['model'] == 'DDPMCondSeg':
        ...
        testing_reviewed(testing_dataset_original_loader, testing_dataset_corrected_loader,
                         args, diffusion=diff, ema=ema, vae=None, test_t=test_t, device=device)
        ...
        testing_reviewed(testing_dataset_original_loader, testing_dataset_corrected_loader,
                         args, diffusion=None, ema=None, vae=model, test_t=test_t, device=device)
