...
def testing_simulated(testing_dataset_loader, args, diffusion, ema, vae, 
                      test_t, error_func, step, device):
    ...
    for batch_idx, data in enumerate(testing_dataset_loader):
        ...
        for im in range(len(keys)):
            ...
            if batch_idx < num_saved_batches:
                if diffusion is not None:
                    plot_name = f"test_{keys[im]}_t{test_t}_{error_func.__name__}_{step}.png"
                    plot_title = f"TEST t={test_t}, {error_func.__name__}, {step}"
                else:
    ...
    if has_image:
        ...
        if diffusion is not None:
            print(f"[IMAGE] MSE t={test_t}, {error_func.__name__}, {step}:")    
        else:
            print(f"[IMAGE] {error_func.__name__}, {step}:")
        print(f"   MSE: Original: {m_ori:.4f} +- {s_ori:.4f}, Simulated: {m_err:.4f} +- {s_err:.4f}")
        ...
        if diffusion is not None:
            filename = f"img_metrics_simulated_v2_{args['dataset']}_{args['test_sampling']}.json"
        else:
    ...
    if has_seg:
        ...
        if diffusion is not None:
            print(f"[SEG] t={test_t}, {error_func.__name__}, {step}:")    
        else:
        ...
        if diffusion is not None:
            filename = f"seg_metrics_simulated_v2_{args['dataset']}_{args['test_sampling']}.json"
        else:
...
def main():
    ...
    if args['model'] == 'DDPM' or args['model'] == 'DDPMCondSeg':
        if args['beta_schedule_gamma'] == 0.25:
            variables["test_t"] = [400, 500, 600]
        elif args['beta_schedule_gamma'] == 0.5:
            variables["test_t"] = [500, 600, 700]
    elif args['model'] == 'VAE':
        variables["test_t"] = [0]
    ...
    test_t = variables["test_t"][t_index]
    simulated_error_func, step = func_step_pairs[pair_index]

    print(f"Running evaluation with test_t={test_t}, simulated_error_func={simulated_error_func}, step={step}")
    ...
    if args['model'] == 'DDPM' or args['model'] == 'DDPMCondSeg':
        ...
        testing_simulated(testing_dataset_loader, args, diffusion=diff, ema=ema, 
                          vae=None, test_t=test_t, error_func=error_functions[simulated_error_func], 
        ...
        testing_simulated(testing_dataset_loader, args, diffusion=None, ema=None, 
                          vae=model, test_t=test_t, error_func=error_functions[simulated_error_func], 
                          step=step, device=device)
